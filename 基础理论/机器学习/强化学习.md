...
\begin{equation}
\begin{aligned}
v_(s)^{k+1}&= v(s)^k - \alpha_k(\widetilde{g}(v_\pi(s)^k))\\
&= v(s)^k - \alpha_k(v_\pi(s)^k-[r_{t+1} + \gamma v_\pi(s')])\\
\end{aligned}
\end{equation}
\tag{4.9}
$$
> $v(s)^k$是$v_\pi(s)$的估计值
...
> sarsa是用来估计action value, 从而进行policy improvement

给定数据/经验{($s_t$, $a_t$, $r_{t+1}$, $s_{t+1}$, $a_{t+1}$)}
> 为啥还要下一时刻的动作$a_{t+1}$呢？因为要计算的是动作价值函数$q(s_t, a_t)$

使用如下Sarsa算法对动作价值函数$q_{t+1}(s_t, a_t)$进行估计:
$$
\begin{equation}
\begin{aligned}
q_{t+1}(s_t, a_t) &= q_{t}(s_t, a_t) - \alpha_t(s_t, a_t)[q_{t}(s_t, a_t) - (r_{t+1} + \gamma q_{t}(s_{t+1}, a_{t+1}))] \\
q_{t+1}(s, a) &= q_{t}(s, a), \forall s \not= s_t, a \not= a_t
\end{aligned}
\end{equation}
\tag{5.1}
$$
...
![Sarsa + policy improvement](../../Resourse/sarsa_policy_improvement.png)
...
\begin{equation}
\begin{aligned}
\mathbb{E}[q_t(s_{t+1}, A)] &= \sum_{a \in A} \pi(a|s_{t+1})q_t(s_{t+1}, a) = v_t(s_{t+1})\\
\end{aligned}
\end{equation}
\tag{5.4}
$$
Expected sarsa算法的本质也是在求一个贝尔曼公式:
$$
\begin{equation}
\begin{aligned}
...
$$
### n-step Sarsa
> 包含了sarsa和mc learning
#### idea
> 与mini-batch stochastic gradient descent思想类似

![n-step Sarsa](../../Resourse/n-step-sarsa.png)
#### n step sarsa算法如下:
$$
\begin{equation}
\begin{aligned}
q_{t+1}(s_t, a_t) &= q_{t}(s_t, a_t) - \alpha_t(s_t, a_t)[q_{t}(s_t, a_t) - (r_{t+1} + + \gamma r_{t+1}+\cdots+\gamma^n q_t(s_{t+n}, a_{t+n}))] \\
q_{t+1}(s, a) &= q_{t}(s, a), \forall s \not= s_t, a \not= a_t
\end{aligned}
\end{equation}
\tag{5.6}
$$
n-step sarsa所需要的数据{($s_t$, $a_t$, $r_{t+1}$, $s_{t+1}$, $a_{t+1}$, $r_{t+2}$, $s_{t+2}$, $a_{t+2}$, $\cdots$, $s_{t+n}$, $a_{t+n}$)}

------

### Q-learning(TD learning of optimal action value)
q-learning算法如下:
$$
\begin{equation}
\begin{aligned}
q_{t+1}(s_t, a_t) &= q_{t}(s_t, a_t) - \alpha_t(s_t, a_t)[q_{t}(s_t, a_t) - (r_{t+1} + \gamma \max_{a \in A} q_t(s_{t+1}, a))] \\
q_{t+1}(s, a) &= q_{t}(s, a), \forall s \not= s_t, a \not= a_t
\end{aligned}
\end{equation}
\tag{5.7}
$$
Q-learning算法与sarsa框架非常相似， 其本质是在求解贝尔曼最优方程:
$$
\begin{equation}
\begin{aligned}
q(s, a) &= \mathbb{E}[R_{t+1} + \gamma \max_{a \in A} q_(S', a)|S_t=s,A_t=a]\\
\end{aligned}
\end{equation}
\tag{5.8}
$$
...
- behavior policy 用来生成经验数据(也即探索过程)
- target policy用来迭代更新策略(也即应用过程)
- off-policy: behavior policy != target policy
- on-policy: behavior policy == target policy 

#### Q-learning implementation(pseudocode)
##### on-policy
![on-policy Q-learning](../../Resourse/q_learning_on_policy_psedocode.png)

##### off-policy
![off-policy Q-learning](../../Resourse/q_learning_off_policy_psedocode.png)

### a unified point of view of TD series learning algorithm
以上所有TD系列算法都可以用一个框架来统一表示:
$$
\begin{equation}
\begin{aligned}
q_{t+1}(s_t, a_t) &= q_{t}(s_t, a_t) - \alpha_t(s_t, a_t)[q_{t}(s_t, a_t) - \overline{q}_t(s_t, a_t)] \\
q_{t+1}(s, a) &= q_{t}(s, a), \forall s \not= s_t, a \not= a_t
\end{aligned}
\end{equation}
\tag{5.9}
$$
其中$\overline{q}_t(s_t, a_t)$的形式对比:
![unified TD series learning compare](../../Resourse/td_comparation.png)
上面所有的算法在数学上都是在求解贝尔曼方程:
![unified TD series learning compare mathmatic](../../Resourse/td_compare_2.png)
