...
|文件系统||
| pwd | 显示当前绝对路径| |
| grep [string]  [文件名] | 搜索文本内的内容| 第二个参数为待匹配内容 - A 3， 显示后3行内容， -B 3 显示前三行内容， -C 3， 前后各显示3行|
...
| rsync - avztP [remote_usr_name@ip_address]:[dir] [本地文件夹] | 将远程电脑中的制定文文件同步到本地，反过来则是推送| 适用于大文件夹内点局域网传输 -a 以归档模式传输， -v显示传输过程中的详细信息， -z压缩传输数据， -P显示传输进度, -t 比较文件时间戳, 指定ssh端口号-e 'ssh -p 8117' |
