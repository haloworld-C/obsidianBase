...
\\ \dot{v} = a
\\ \dot{\alpha} = \frac{v*tan(\delta)}{L} - \omega_{trailer}
\end{matrix}\right.$$
> 这里我们把$\alpha$当作已知量， 实际使用中有可能不可测量(其实可以很方便的由激光数据得到或通过其他传感器测量)， 需要对该值进行估计。
> 拖挂的空间位置可由状态$x, y, {\phi}, {\alpha}$及车辆尺寸(常数)唯一确定。

> 另一种基于速度分解的方法更简洁一点(本质是一样的)， [供参考](https://zhuanlan.zhihu.com/p/592384422)
#### 多车运动学
