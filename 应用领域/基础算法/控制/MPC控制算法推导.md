...
> 1. 前提是H矩阵可逆 ，不可逆的情况下可以使用数值方法求其近似解(可以看出， 当前的控制输入是当前状态的全状态反馈， 这种情况下MPC等价于LQR只是求解方式不同)
...
\end{equation}
$$
> 形式二上在实现上比较简单， 直接把数据准备好交给求解器就好了。但是由于其维度比形式一大了很多， 所以其求解稳定性没有形式一好。而且理论上形式一的矩阵系数可以提前计算， 所以实时性及稳定性上要优于形式二
### 优化目标求解 
