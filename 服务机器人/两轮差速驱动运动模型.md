...
#####  讨论
1. 差速机器人进行跟踪的最小转弯半径
有上面正运动学公式, 差速机器人圆周运动时的转弯半径：
$$
R=\frac{H(v_1+v_2)}{2(v_1-v_2)}=\frac{H(v_1-v_2)+2Hv_2}{2(v_1-v_2)}=H(\frac{1}{2}+\frac{1}{\frac{v_1}{v_2}-1})
$$
不妨另$t=\frac{v_1}{v_2}$, $t$的取值范围需要进一步讨论：
- 当$v_1, v_2$不等于0且异号时, 此时机器人的受力中心为机器人中心，机器人自转，机器人中心线速度为0;
- 当$v_1, v_2$有一个为零时, 此时机器人的受力中心仍为机器人中心，机器人自转，机器人中心线速度为0;
故$t{\in}(0, +\infty)$, 机器人做圆周运动的转弯半径：
- 当$t->1$时，差动机器人的圆心位于无穷远处$R->\infty$， 机器人作直线运动
- 当$t->\infty$时，差动机器人转弯半径$R->\frac{H}{2}$
- 当$t->0$时， 差动机器人的转弯半径$R->-\frac{H}{2}$
综合以上分析可知差动机器人转弯半径$R\in(-\infty, -\frac{H}{2}), (\frac{H}{2}, +\infty)$， 故差动机器人做圆周运动的最小转弯半径为：
$$
|R_min| = \frac{H}{2}
$$
对应最大的曲率为:
